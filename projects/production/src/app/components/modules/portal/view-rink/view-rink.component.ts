...
export class ViewRinkComponent implements OnInit {
  ...
  productionId = localStorage.getItem('production_id');
  rink: any;
  ...
  getRink(){
    this.portalApi.getSingleRink(sessionStorage.getItem('production_rink_id'))
      .subscribe(
        res => {
          console.log(res);
          this.rink = res;
          sessionStorage.setItem('production_rink_source_id', res.rink_source)
