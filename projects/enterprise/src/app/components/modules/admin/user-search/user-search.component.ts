...
export class UserSearchComponent implements OnInit {
  ...
  getDetail(){
    this.adminApi.getDetail(String(sessionStorage.getItem('enterpriseAdminSearchUser')))
      .subscribe(
        res => {
          console.log(res);
  ...
  sendInvitation() {
    let data = {
      account: localStorage.getItem('enterprise_id'),
    ...
    this.adminApi.sendInvitation(data)
      .subscribe(
        res => {
          console.log(res);
          sessionStorage.setItem('enterprise_invitation_id', res.data.id);
