...
export class NewRinkComponent implements OnInit {
  ...
  sendRink(){
    let rinkData = {
      sender: localStorage.getItem('enterprise_id'),
      recipient: sessionStorage.getItem('enterprise_rink_recipient'),
    ...
    this.portalApi.postRink(rinkData)
      .subscribe(
        res => {
          ...
          if (res.message == "OK"){
            sessionStorage.setItem('enterprise_rink_id', res.data.id);
