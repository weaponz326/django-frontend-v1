...
export class NewRinkComponent implements OnInit {
  ...
  sendRink(){
    let rinkData = {
      sender: localStorage.getItem('association_id'),
      recipient: sessionStorage.getItem('association_rink_recipient'),
    ...
    this.portalApi.postRink(rinkData)
      .subscribe(
        res => {
          ...
          if (res.message == "OK"){
            sessionStorage.setItem('association_rink_id', res.data.id);
